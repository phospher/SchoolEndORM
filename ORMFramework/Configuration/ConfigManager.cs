...
namespace ORMFramework.Configuration {
    public class ConfigManager {
            ...
            if ( reader.Read () && reader.NodeType == XmlNodeType.XmlDeclaration ) {
                if ( reader.Read () && reader.NodeType == XmlNodeType.Element && reader.Name == "Configuration" ) {
                    while ( result && reader.Read () && reader.NodeType == XmlNodeType.Element ) {
                        switch ( reader.Name ) {
                            case "SessionFactory": result = ReadSessionFactory ( reader, configuration );
                                break;
                            case "Mappings": result = ReadMappings ( reader, configuration );
                                break;
                            default: break;
                        }
                    }
        ...
        private bool ReadSessionFactory ( XmlTextReader reader, Configuration configuration ) {
            ...
            while ( result && reader.Read () && reader.NodeType == XmlNodeType.Element ) {
                switch ( reader.Name ) {
                    case "Database": result = ReadDatabase ( reader, configuration );
                        break;
                    case "ConnectionString": result = ReadConnectionString ( reader, configuration );
                        break;
        ...
        private bool ReadDatabase ( XmlTextReader reader, Configuration configuration ) {
            ...
            if ( reader.Read () && reader.NodeType == XmlNodeType.Text ) {
                switch ( reader.ReadString ().ToLower () ) {
                    case "sqlserver": configuration.DatabaseType = DatabaseType.SQLServer;
                        result = true;
                        break;
                    case "odbc": configuration.DatabaseType = DatabaseType.Odbc;
                        result = true;
                        break;
                    case "oledb": configuration.DatabaseType = DatabaseType.OleDb;
                        result = true;
                        break;
                    case "oracle": configuration.DatabaseType = DatabaseType.Oracle;
                        break;
                    default: return false;
                }
            }
        ...
        private bool ReadMap ( XmlTextReader reader, Configuration configuration ) {
            ...
            for ( int i = 0; i < mapNode.ChildNodes.Count; i++ ) {
                ...
                if ( node.Name == "Many-to-Many" ) {
                    ...
                    switch ( node.Name ) {
                        case "One-to-Many": relation.Type = RelationType.OneToMany;
                            break;
                        case "One-to-One": relation.Type = RelationType.OneToOne;
                            break;
                        case "Many-to-One": relation.Type = RelationType.ManyToOne;
                            break;
                        default: return false;
                    }
        ...
        private EntityRelation GetManyToManyRelation ( XmlNode node ) {
            ...
            for ( int i = 0; i < node.ChildNodes.Count; i++ ) {
                ...
                switch ( childNote.Name ) {
                    case "ReferenceClass": relation.ReferenceClassName = childNote.Attributes["Name"].Value;
                        relation.ReferenceClassKeyColum = childNote.Attributes["KeyColum"].Value;
                        break;
                    case "ReferenceTable": relation.ReferenceTableName = childNote.Attributes["Name"].Value;
